...
- 0.91.0 — ADR-058: Why AI over a dedicated engineer for this project. Documents the realistic comparison: AI makes fast/cheap/documented mistakes vs human makes slow/expensive/undocumented mistakes. Three vendors, 25M+ rows, 58 ADRs built for less than one month of an engineer's salary. The defensible position: the platform shipped, the knowledge is captured, the cost is fractional.
- 0.90.0 — ADR-057: Shopify timezone truth and count reconciliation. Eight layers of debugging: backfill filter → sort key collisions → timezone boundaries → Shopify inclusive < operator → <=last_day as correct API filter → DST-aware AT TIME ZONE in Postgres. All 2016-2018 verified: 100% match. Business truth is created_at:<=last_day, not <first_of_next_month.
- 0.89.0 — ADR-056: AI blind spots in data pipeline engineering. Five systematic failure patterns documented with checkpoints for every future session. The rule: the AI builds, the operator verifies. Every backfill must be count-verified against the source.
- 0.88.0 — ADR-055: Shopify backfill filter bug and gap repair. The updated_at:>= filter skipped 5.2% of orders and 65.7% of customers — records never modified after creation were permanently invisible. Fix deployed, repair scripts for both streams running in parallel. Repair optimized: pre-loads existing IDs, skips S3 writes for known records, 250/page, self-contained GraphQL with month filter on every page. Meta-rule added: when a pagination bug is found in one client, audit all others immediately.
- 0.87.0 — Fixed root cause of Shopify 390K order gap: updated_at:>= filter during backfill permanently skipped all records with updated_at before the checkpoint. Fix: during backfill (page_cursor exists), use only GraphQL endCursor for continuation, no timestamp filter. Filter only activates in incremental mode. Updated ADR-049 with third pagination bug variant. Added gap repair script.
...
- 0.82.0 — Shopify GraphQL fixes: updated refunds/transactions from connection types (edges/node) to direct arrays for 2026-04 API, bumped all Shopify streams to api_version 2026-04, fixed totalRefundedSet field name (was incorrectly changed to totalSet), added default=str to all json.dumps calls in pg_client (Decimal serialization failure on every Shopify order). TODO: fail-fast on repeated structural errors to prevent log flooding.
- 0.81.0 — ADR-051: Shopify staged rollout — backfill before webhooks. Webhooks not enabled until polling is complete, stable for 3+ days, and snapshotted. Documents four specific risks of webhooks during backfill: race conditions, unproven code path, Aurora capacity, and rebuild cost without a snapshot.
- 0.80.0 — Shopify client updated to 2026 client_credentials OAuth flow. Auto-detects auth mode: tries client_id + client_secret from SSM (24h rotating tokens), falls back to legacy static access_token. Token cached and auto-refreshed 5 min before expiry. User-Agent header added (ADR-045). Backward compatible with existing shpat_ tokens.
- 0.79.0 — ADR-050: Gorgias first deployment lessons. Five failures documented (User-Agent block, Pydantic field type, ascending/descending pagination gap, API cursor not persisted, lock contention). Three failures prevented by prior ADRs. Eight-step deployment rule for future streams. Fixed Gorgias cursor to persist full API pagination state across runs.
- 0.78.0 — Yotpo seed complete. 254K reviews, 805 products, 104K metadata (41% — MySQL ceiling, 150K reviews have empty metadata at source). Reconciliation: 990/1000 match, mismatches are emoji encoding and verified_buyer null handling only. Gorgias backfilling ascending, 82K tickets and growing. Seed script now only disables Yotpo EventBridge rules, not all streams. Fixed S3 writer region.
- 0.77.0 — ADR-049: backfill pagination must be ascending until current. Gorgias lost ~400K tickets due to premature switch from ascending to descending pagination after first run. Fixed: client stays ascending until checkpoint is within 24h of now, then switches to descending for incremental. Truncated and restarted Gorgias backfill. Rule applies to all future polling clients.
- 0.76.0 — ADR-048: why ADRs are the product, not the code. Documents the economic shift that made ADR-driven development viable (AI collapsed writing cost, AI became the reader), evidence from this platform (47 ADRs, 5-7x ROI in first week), and why skipping documentation was rational before AI and irrational after.
- 0.75.0 — ADR-047: parallel stream validation playbook. Seven-step check sequence for validating multiple streams simultaneously: integrity, metadata, geographic distribution, product gaps, cursor state, cross-stream tickets, lock contention. Documented first parallel deployment results (Yotpo rebuild + Gorgias first run). Fixed Gorgias via field type (dict→Any) for tickets imported from Zendesk.
