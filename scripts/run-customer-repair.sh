...
q = 'query { customersCount(limit: null, query: "created_at:<=' + f'{now.year}-{now.month:02d}-{last_day:02d}' + '") { count } }'
...
with urlopen(req, timeout=30) as resp:
    ...
    print(body.get('data', {}).get('customersCount', {}).get('count', '?'))
PYEOF
)
...
while true; do
  ...
  RESULT=$(cat /tmp/customer-repair-result.json 2>/dev/null || echo "{}")
