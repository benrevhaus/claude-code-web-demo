...
function applyFilters<T extends Record<string, string | number | boolean>>(rows: T[], filters: Partial<Filters>, searchKeys: string[]) {
  return rows.filter((row) => {
    if (filters.startDate && String(row.date_pst) < filters.startDate) return false;
    if (filters.endDate && String(row.date_pst) > filters.endDate) return false;
    if (filters.pagePath && row.page_path !== filters.pagePath) return false;
    if ("landing_page_path" in row && filters.landingPagePath && row.landing_page_path !== filters.landingPagePath) return false;
    if ("event_name" in row && filters.eventNames) {
      if (filters.eventNames.length === 0) return false;
      if (!filters.eventNames.includes(String(row.event_name))) return false;
    }
    if ("raw_event_name" in row && filters.rawEventName && row.raw_event_name !== filters.rawEventName) return false;
    if ("event_class" in row && filters.eventClass && row.event_class !== filters.eventClass) return false;
    if (filters.deviceCategory && row.device_category !== filters.deviceCategory) return false;
    if (filters.sourceMedium && row.source_medium !== filters.sourceMedium) return false;
    if ("is_conversion_event" in row && filters.conversionOnly === "true" && row.is_conversion_event !== true) return false;
    if (filters.search) {
...
function sortRows<T extends Record<string, string | number | boolean>>(rows: T[], sortBy: string, sortDirection: "asc" | "desc") {
...
function groupRows<T extends Record<string, string | number | boolean>>(
  rows: T[],
  groupingKeys: string[],
  metricKeys: string[],
) {
  ...
  for (const row of rows) {
    ...
    for (const metricKey of metricKeys) {
      (existing as Record<string, string | number | boolean>)[metricKey] =
...
export function mockApi(
  ...
  const filters = params as Partial<Filters> | undefined;
  ...
  if (path === "/pages") {
    ...
    const filtered = sortRows(
      groupRows(
        applyFilters(dataset.pageRows, filters ?? {}, ["page_path", "page_title", "landing_page_path"]),
        groupingKeys,
        ["views", "sessions", "total_users", "event_count"],
      ),
      String(params?.sortBy ?? "date_pst"),
  ...
  if (path === "/events") {
    ...
    const filtered = sortRows(
      groupRows(
        applyFilters(dataset.eventRows, filters ?? {}, ["page_path", "event_name", "raw_event_name", "derived_page_path"]),
        groupingKeys,
