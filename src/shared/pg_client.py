...
class PgClient:
    ...
    def upsert_order(self, order: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        params = {
            ...
            "tags": data.get("tags"),
            "note": data.get("note"),
            "line_items": json.dumps(data.get("line_items"), default=str) if data.get("line_items") else None,
            "shipping_address": json.dumps(data.get("shipping_address"), default=str) if data.get("shipping_address") else None,
            "billing_address": json.dumps(data.get("billing_address"), default=str) if data.get("billing_address") else None,
            "raw_s3_key": s3_key,
    ...
    def upsert_ticket(self, ticket: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        params = {
            ...
            "spam": data.get("spam"),
            "via": json.dumps(data.get("via"), default=str) if data.get("via") else None,
            "customer": json.dumps(data.get("customer"), default=str) if data.get("customer") else None,
            "assignee_user": json.dumps(data.get("assignee_user"), default=str) if data.get("assignee_user") else None,
            "tags": data.get("tags"),
            "raw_s3_key": s3_key,
            "schema_version": schema_version,
            "run_id": run_id,
    ...
    def upsert_customer(self, customer: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        params = {
            ...
            "default_address": json.dumps(data.get("default_address"), default=str) if data.get("default_address") else None,
            "addresses": json.dumps(data.get("addresses"), default=str) if data.get("addresses") else None,
    ...
    def upsert_product(self, product: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        params = {
            ...
            "variants": json.dumps(data.get("variants"), default=str) if data.get("variants") else None,
            "images": json.dumps(data.get("images"), default=str) if data.get("images") else None,
            "raw_s3_key": s3_key, "schema_version": schema_version, "run_id": run_id,
        }
        with self._conn.cursor() as cur:
            cur.execute(sql, params)
    ...
    def upsert_refund(self, refund: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        params = {
            ...
            "refund_line_items": json.dumps(data.get("refund_line_items"), default=str) if data.get("refund_line_items") else None,
            "raw_s3_key": s3_key, "schema_version": schema_version, "run_id": run_id,
        }
        with self._conn.cursor() as cur:
            cur.execute(sql, params)
    ...
    def upsert_review(self, review: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        sql = """
            INSERT INTO yotpo.reviews_raw_current (
                ...
                votes_up, votes_down, product_yotpo_id, domain_key, product_name,
                reviewer_type, verified_buyer, is_incentivized, incentive_type,
                source_review_id, images_data, name, email,
                deleted, archived, created_at, updated_at, raw_payload,
                raw_s3_key, schema_version, run_id
            ) VALUES (
                %(id)s, %(store_id)s, %(score)s, %(title)s, %(content)s, %(sentiment)s,
                %(votes_up)s, %(votes_down)s, %(product_yotpo_id)s, %(domain_key)s, %(product_name)s,
                %(reviewer_type)s, %(verified_buyer)s, %(is_incentivized)s, %(incentive_type)s,
                %(source_review_id)s, %(images_data)s, %(name)s, %(email)s,
                %(deleted)s, %(archived)s, %(created_at)s, %(updated_at)s, %(raw_payload)s,
                %(raw_s3_key)s, %(schema_version)s, %(run_id)s
            )
            ON CONFLICT (id, store_id) DO UPDATE SET
                ...
                product_yotpo_id = COALESCE(EXCLUDED.product_yotpo_id, yotpo.reviews_raw_current.product_yotpo_id),
                domain_key = COALESCE(EXCLUDED.domain_key, yotpo.reviews_raw_current.domain_key),
                product_name = COALESCE(EXCLUDED.product_name, yotpo.reviews_raw_current.product_name),
                reviewer_type = EXCLUDED.reviewer_type, verified_buyer = EXCLUDED.verified_buyer,
                is_incentivized = EXCLUDED.is_incentivized, incentive_type = EXCLUDED.incentive_type,
        ...
        params = {
            ...
            "is_incentivized": data.get("is_incentivized", False),
            "incentive_type": data.get("incentive_type"),
            "source_review_id": data.get("source_review_id"),
            "images_data": json.dumps(data.get("images_data", []), default=str),
            "name": data.get("name"),
            "email": data.get("email"),
    ...
    def upsert_ga4_event(self, event: BaseModel, s3_key: str, schema_version: str, run_id: Optional[str] = None) -> bool:
        ...
        params = {
            ...
            "params": json.dumps(data.get("params"), default=str) if data.get("params") else None,
            "items": json.dumps(data.get("items"), default=str) if data.get("items") else None,
            "user_properties": json.dumps(data.get("user_properties"), default=str) if data.get("user_properties") else None,
            "raw_s3_key": s3_key,
            "schema_version": schema_version,
            "run_id": run_id,
        }
